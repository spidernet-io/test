                    ...
                    int (*okfn)(struct sk_buff *))
{
    ...
    // for vxlan traffic, DO NOT traverse netfilter
    ip1 = ip_hdr(skb);
    if (NULL != ip1) {
        if (IPPROTO_UDP == ip1->protocol) {
            udp_header = (struct udphdr *)skb_transport_header(skb);
            if (ntohs(udp_header->dest) == 4789 || ntohs(udp_header->source) == 4789) {
                return NF_STOP;
            }
        }
    }
...
int init_module()
{
    ...
    printk("init_module,fastpath_local_out\n");
    ...
    printk("init_module,fastpath_post_routing\n");
    ...
    printk("init_module,fastpath_pre_routing\n");
    ...
    printk("init_module,fastpath_local_in\n");
